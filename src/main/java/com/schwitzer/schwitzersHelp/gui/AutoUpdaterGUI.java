package com.schwitzer.schwitzersHelp.gui;
...
public class AutoUpdaterGUI extends GuiScreen {
    ...
    private static final String GITHUB_API_URL = "https://api.github.com/repos/ARandomGuyOutSide/SchwitzersHelp/releases";
                    ...
                    executor.submit(() -> {
                        try {
                            ...
                            downloadFileWithProgress(
                                    downloadURL,
                                    new File(mc.mcDataDir + "/mods/SchwitzersHelp-" + latestVersion + ".jar")
                            );
                            mc.addScheduledTask(() -> {
                                downloadBtn.enabled = false;
    ...
    private class ScrollableList extends GuiSlot { // spent way too much time on this - yuro
        ...
        protected void drawSlot(int entryID, int insideLeft, int yPos, int insideSlotHeight, int mouseXIn, int mouseYIn) {
            if (entryID >= 0 && entryID < splitReleaseMessage.size()) {
                String item = format(splitReleaseMessage.get(entryID));
                ScaledResolution res = new ScaledResolution(mc);
    ...
    private static String format(String markdownText) {
        // Bold formatting
